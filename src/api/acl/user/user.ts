...
export default {
  ...
  reqAddOrUpdateUser(addOrUpdateUserParanms:ReqAddOrUpdateUser) {
    if(addOrUpdateUserParanms.id){
      return request.put<any,any>(AclApi.UPDATEUSER_URL,addOrUpdateUserParanms)
    }else{
  ...
  reqUserPageList(page: number, limit: number, userName?: string) {
    return request.get<ResUserPageData<any>, any>(
      `${AclApi.USERLIMIT_URL}/${page}/${limit + (userName ? '?username=' + userName : '')}`,
    )
  },
  // 删除单个用户
  ...
  reqUserRoles(id:number){
    return request.get<ResUserRolesListData,any>(`${AclApi.USERROLES_URL}/${id}`)
  },
  // 分配用户角色
  reqUserAssignRoles(assignRolesParams:UserAssignRolesData){
    return request.post<null,any>(AclApi.USERASSIGNROLES_URL,assignRolesParams)
  }
}
