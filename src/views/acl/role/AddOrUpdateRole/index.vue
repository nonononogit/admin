...
const confirmAddOrUpdateRole = async () => {
  ...
  if (
    !!props.currentRole.roleName &&
    props.currentRole.roleName === roleForm.value.roleName
  )
    return
...
// 监听父组件传过来的数据
watch(
  [() => props.currentRole, dialogRoleFormVisible],
  (newVal) => {
