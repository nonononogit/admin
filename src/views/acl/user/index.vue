<template>
  <div class="acl-container">
    <el-card class="top-card">
      <span>用户名：</span>
      <el-input v-model="userName" placeholder="请输入用户名" class="ipt" clearable></el-input>
      <div class="btn">
        <el-button type="primary" :disabled="!userName" @click="searchUserOrReset(userName)">搜索</el-button>
        <el-button type="primary" @click="searchUserOrReset(undefined)">重置</el-button>
      </div>
    </el-card>
    <el-card class="bottom-card">
      <div class="btn">
        <el-button type="primary" @click="addOrUpdateUser">添加用户</el-button>
        <el-button type="primary" :disabled="!userArr.length" @click="deleteBatchUser">批量删除</el-button>
      </div>
      <el-table class="table" :header-cell-style="{ textAlign: 'center' }" :cell-style="{ textAlign: 'center' }"
        :data="userPageList.records" border ref="multipleTableRef" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="50" />
        <el-table-column type="index" label="#" width="50">
        </el-table-column>
        <el-table-column prop="id" label="ID" width="80">
        </el-table-column>
        <el-table-column prop="username" label="用户名字" width="100" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="name" label="用户名称" width="100" show-overflow-tooltip>
        </el-table-column>
        <el-table-column prop="roleName" label="用户角色" width="120" show-overflow-tooltip>
        </el-table-column>
        ...
        <el-table-column prop="" label="操作" width="">
          <template #default="scoped">
            <el-button :icon="User" type="primary" size="small" @click="assignRoles(scoped.row)">分配角色</el-button>
            <el-button :icon="Edit" type="warning" size="small" @click="addOrUpdateUser(scoped.row)">编辑</el-button>
...
const confirmDelete = async (id: number) => {
  ...
  aclStore.getUserPageList(userPageList.value.records.length < 1 ? currentPage.value - 1 : currentPage.value, pageSize.value)
}
// 批量删除
