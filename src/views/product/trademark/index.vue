<template>
  <div class="trademark-container">
    <el-card>
      <el-button type="primary" @click="addOrUpdateTrademarkEvent" :icon="Plus">
        添加品牌
      </el-button>
      <el-table
        :data="trademarkLimitData.records"
        border
        ...
        <el-table-column label="操作">
          <template #="{ row }">
            <el-button
              ...
              :icon="Edit"
              size="small"
              @click="addOrUpdateTrademarkEvent(row)"
            ></el-button>
            <el-popconfirm
              :title="`确定要删除${row.tmName}吗？`"
              @confirm="confirmDelTm(row.id)"
            >
              <template #reference>
                <el-button
                  type="danger"
      ...
      <el-pagination
        ...
        :total="trademarkLimitData.total"
        @size-change="handleSizeChange"
        @current-change="getTmList"
      />
    </el-card>
    <addOrUpdateTrademark
      ref="addOrUpdateTrademarkRef"
      :updateTm="currentTm"
      :limit="limit"
    ></addOrUpdateTrademark>
  </div>
...
const addOrUpdateTrademarkEvent = (row?: RecordsData) => {
  ...
  limit.value.currentPage = row?.id
    ? currentPage.value
    : (currentPage.value = 1)
...
const confirmDelTm = async (id: number) => {
  try {
    ...
    getTmList(
      trademarkLimitData.value.records.length === 1
        ? currentPage.value - 1
        : currentPage.value,
    )
  } catch (error) {
